...
int SP_HttpHandlerAdapter::handle(SP_Request* request, SP_Response* response)
{
	...
	if(NULL != httpResponse->getHeaderValue(SP_HttpMessage::HEADER_CONNECTION))
	{
		strncpy(keepAlive, httpResponse->getHeaderValue(SP_HttpMessage::HEADER_CONNECTION), sizeof(keepAlive) - 1);
	}

	if(NULL != httpResponse->getContent())
	{
		response->getReply()->getFollowBlockList()->append(new SP_HttpResponseMsgBlock(httpResponse));
	}
	else
	{
		delete httpResponse;
